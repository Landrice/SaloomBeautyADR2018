...
public class CustomerBLL {
    ...
    public void saveType(Customer customer){
        if(isUniqNameType(customer)){
            customerGetway.saveType(customer);
        }
    }
    ...
    public boolean isUniqNameType(Customer customer) {
        boolean inUniqName = false;
        try {
            pst = con.prepareStatement("select * from "+db+".Customerstype where CustomersType=?");
            pst.setString(1, customer.customerName);
            rs = pst.executeQuery();
            while (rs.next()){
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
