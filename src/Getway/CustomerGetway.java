...
public class CustomerGetway {
    ...
    public void save(Customer customer) {
        ...
        try {
            ...
            pst.setString(8, customer.customertypeId);
            pst.executeUpdate();
            pst.close();
            con.close();
    ...
    public void saveType(Customer customer) {
        con = dbCon.geConnection();
        try {
            pst = con.prepareStatement("insert into "+db+".Customerstype values(?,?,?,?,?,?)");
            pst.setString(1, null);
            pst.setString(2, customer.customertype);
            pst.setString(3, customer.userId);
            pst.setString(4, customer.date);
            pst.setString(5, customer.customerdiscountservice);
            pst.setString(6, customer.customerdiscountproducts);
            pst.executeUpdate();
            pst.close();
            con.close();
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
    ...
    public void view(Customer customer) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                customer.customertypeId=rs.getString(8);
                customer.userName = sql.getName(customer.userId, customer.userName, "User");
                customer.customertype = sql.getName(customer.customertypeId, customer.customertype, "Customerstype");
                customer.customerList.addAll(new ListCustomer(customer.id, customer.customerName, customer.customerConNo, customer.customerAddress, customer.totalBuy, customer.userName, customer.date,customer.customertype));
            }
            rs.close();
            pst.close();
            con.close();
    ...
    public void update(Customer customer) {
        con = dbCon.geConnection();
        try {
            pst = con.prepareStatement("UPDATE "+db+".Customer set CustomerName=?,CustomerContNo=?,CustomerAddress=?,customerId=? where Id=?");
            pst.setString(1, customer.customerName);
            pst.setString(2, customer.customerConNo);
            pst.setString(3, customer.customerAddress);
            pst.setString(3, customer.customertypeId);
